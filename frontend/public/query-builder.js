...
CampusExplorer.buildQuery = function () {
    ...
    function conditions(query, kind, tab) {
        ...
        for (let cond of conditions) {
            ...
            if (condition_operator !== "IS") {
                condition_term = parseInt(condition_term, 0);
            }
            let condition = {};
    ...
    function apply(query, kind, tab) {
        ...
        for (let trans of transformations) {
            ...
            for (let operator of operators) {
                if (operator.selected) {
                    tOperator = operator.getAttribute("value");
                }
            }
            ...
            for (let option of options) {
                if (option.selected) {
                    tOption = option.getAttribute("value");
                }
            }
            let transform = {};
            let value = kind + tOption;
            transform[tOperator] = value;
            let transformation = {};
